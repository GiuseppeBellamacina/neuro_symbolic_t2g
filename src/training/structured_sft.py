...
STATUS ON THIS BRANCH: **library with no training client.**

Recovered verbatim from the ``edit-rewards`` branch (commit ``68bc12a``) because
the mathematics is validated (see ``tests/test_structured_loss.py`` and
``tests/test_structured_transitions.py``, brute-force cross-checked). The
trainer integration from that branch — ``auxiliary_sft_trainer.py`` and
``auxiliary_sft_model.py`` — was deliberately **NOT** recovered: it changed
default-path SFT (fingerprint v1->v4, the bigram artifact was no longer
produced, and historical configs lacking an ``experiment:`` block became
unrunnable). Keeping the mathematics while rejecting that trainer means there is
currently **no entry point that calls this module**; the only callers are its
tests.

Do not read "opt-in" as "switchable from a config": no config switch exists.
Before using this in training, run GATE 2 in ``docs/NEW_OBJECTIVES_SPEC.md``
(head-only benchmark against a shuffled-transition control).

Expected value on ASLG-PC12 is low and was measured, not assumed: the source
word alone removes ~90% of gloss uncertainty and the first-order structural
prior is worth ~5% of the remaining residual on length-aligned pairs. This
module becomes scientifically interesting on a corpus where gloss is not a
shallow function of the source (e.g. PHOENIX-2014T).
"""
