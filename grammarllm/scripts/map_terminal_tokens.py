    ...
    def check_tokens_conflicts(table_parsing, map_terminal_tokens):
        ...
        for lhs, rhs_list in table_parsing.items():
            ...
            for a, b in itertools.combinations(rhs_list.keys(), 2):
                ...
                if intersection:
                    logging.info(
                        f"Conflitto tra '{a}' e '{b}': {intersection}"
                    )  # debug
                    conflicts.append(
                        f"I set di tokens associati ai terminali '{a}' e '{b}' non sono disgiunti. Intersezione: {list(intersection)[:5]}"
                    )
