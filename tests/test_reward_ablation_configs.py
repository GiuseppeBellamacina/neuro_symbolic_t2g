...
def test_reward_ablations_are_one_factor_manual_configs():
    ...
    for path in configs:
        ...
        assert set(raw) == {"extends", "experiment", "reward"}
    ...
    assert names == set(REWARD_NAMES) - {"edit-validity"}
    assert parent["reward"]["name"] == "edit-validity"
    assert names | {parent["reward"]["name"]} == set(REWARD_NAMES)
