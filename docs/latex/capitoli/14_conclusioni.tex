...
reinforcement learning possa agire. Con l'SFT a exact match $0{,}8117$ e validity
...
SFT porta ROUGE-L da $0{,}9749$ a $0{,}5114$ ed exact match da
$0{,}8117$ a $0{,}0203$. Il meccanismo è che gli aggiornamenti avvengono
...
\begin{itemize}
  \item alza la non-copy-token accuracy da $0{,}0006$ a $0{,}0432$, un fattore
        circa $70$;
  \item alza la validity da $0{,}0370$ a $0{,}9055$, un fattore $24$;
...
Quattro ritrattazioni misurate sono documentate in questa relazione, e sono
...
\begin{enumerate}
  ...
  \item Il vantaggio dell'SFT sulla baseline a regole misurato con ROUGE-L è
        stato \textbf{declassato}. Una precedente stesura lo presentava come
        statisticamente solido sulla base di un intervallo bootstrap che esclude
        lo zero. Il confronto fra le otto celle eseguite due volte mostra però
        che la dispersione dovuta al solo addestramento eguaglia o supera quegli
        intervalli (sezione~\ref{sec:dispersione-run}): il delta su ROUGE-L
        ($+0{,}0064$) non è distinguibile dal rumore, mentre quello su exact
        match ($+0{,}2252$) resta robusto.
  \item Un meccanismo di \textbf{curriculum} per difficoltà, presente in una
        fase intermedia del progetto, è stato \textbf{rimosso} dopo aver
        dimostrato che era inerte: i suoi stadi risultavano indistinguibili
        sulla classe di difficoltà che intendevano graduare, perché i target
        dichiarati eccedevano la disponibilità del corpus e il codice troncava
        in silenzio al massimo disponibile. In aggiunta introduceva una
        frazione elevata di esempi ripetuti negli stadi avanzati.
\end{enumerate}

Sull'ultimo punto vale la pena estrarre una lezione trasferibile, perché il
modo in cui il difetto è rimasto nascosto è più interessante del difetto stesso.
Quel componente aveva test che passavano e un'implementazione priva di errori di
programmazione. Ciò che mancava era una verifica che il meccanismo producesse
l'effetto \emph{dichiarato} sui dati \emph{reali}: i test misuravano le
proporzioni su campioni di poche centinaia di righe, dove il limite di
disponibilità non si manifesta. La domanda ``questo componente fa quello che
dice?'' non era mai stata posta in una forma verificabile alla scala di
produzione.

Alla stessa categoria appartiene una correzione di coerenza interna. Una
precedente stesura della tabella principale mescolava valori prodotti da due
versioni diverse del modulo delle metriche, con l'effetto che le colonne di
sovrapposizione e quella di validity provenivano da codici differenti, e che
celle diverse pescavano da run differenti. Le tabelle sono state ricostruite da
fonte unica, dichiarata nella sezione~\ref{sec:provenienza}. L'errore è
segnalato perché la sua correzione ha spostato la validity della cella senza
vincolo da $0{,}1043$ a $0{,}0370$, cioè ha \emph{rafforzato} il risultato
sul vincolo simbolico portando il fattore di miglioramento da circa $9$ a $24$.

Alla stessa categoria appartiene una quinta correzione, la più imbarazzante
perché riguarda una diagnosi e non una misura. Una precedente stesura dichiarava
un test permanentemente rosso e ne attribuiva il fallimento a una dipendenza
dalla rete. Leggendo il corpo del test si è visto che non apre alcuna
connessione: sostituisce il meccanismo di esecuzione con un oggetto che cattura
il comando costruito e ne verifica la forma
(sezione~\ref{sec:test-ssh-ritrattazione}). La causa era stata inferita dal
\emph{nome} del test invece che dal suo contenuto. La suite è interamente verde,
e conta $476$ test.

L'errore è dello stesso tipo dei cinque guasti infrastrutturali del
Capitolo~\ref{cap:infrastruttura}: una spiegazione plausibile adottata senza
verifica. Che si sia insinuato nel documento che quei guasti descrive è la
migliore illustrazione di quanto il meccanismo sia difficile da evitare.
...
I due obiettivi del Capitolo~\ref{cap:ausiliari} sono ora addestrati
(sezione~\ref{sec:osservazione-ausiliari}), ma nessuno dei due gate è ancora
chiuso:

\begin{enumerate}
  \item la \textbf{massa ammessa} migliora l'exact match di $+0{,}068$ in
        valutazione campionata, contro la previsione dichiarata in anticipo.
        Restano da escludere l'effetto di calibrazione (valutazione greedy, in
        corso) e la maggiore durata dell'addestramento; la sonda in inferenza
        sulla massa rimossa, criterio originale del gate, non è ancora stata
        calcolata;
  \item la \textbf{loss strutturata} peggiora tutte le metriche rispetto
        all'SFT, fermandosi molto prima per early stopping. Il confronto con il
        controllo mescolato corretto, che conserva il supporto del grafo e
        permuta solo i pesi, è in corso: la prima versione del controllo non
        misurava ciò che doveva.
\end{enumerate}

Per entrambi vale la raccomandazione della sezione~\ref{sec:dispersione-run}:
con un solo seed per cella, un vantaggio di pochi punti di exact match va
confermato con più seed prima di essere considerato stabilito. Il valore
$+0{,}0444$ bit della qualificazione strutturale continua a suggerire che il
margine disponibile per la loss strutturata sia modesto.
