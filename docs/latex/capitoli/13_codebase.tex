...
\texttt{ablations/objectives} & \texttt{sft-allowed-mass} & addestramento \\
                              ...
                              & \texttt{sft-structured-shuffled} & addestramento \\
                              & \texttt{sft-allowed-mass-greedy} & sola valutazione \\
\texttt{sft} & \texttt{zero-shot-greedy} & sola valutazione \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{Le celle appaiate e il loro isolamento}
\label{sec:celle-non-isolate}

L'intenzione dichiarata della matrice è che le celle \texttt{zero-shot} e
\texttt{few-shot} differiscano soltanto per la modalità di prompting. Un
confronto sistematico delle configurazioni \emph{risolte} mostra due
differenze:
...
\midrule
\texttt{retrieval.enabled} & \texttt{false} & \texttt{true} (con $k=3$) \\
\texttt{max\_prompt\_length} & $256$ & $768$ \\
\bottomrule
\end{tabular}
\end{center}

Le due sono però solidali, non indipendenti: gli esempi recuperati allungano il
prompt, quindi attivare il retrieval \emph{richiede} di alzare la lunghezza
massima. Se non si alzasse, gli esempi verrebbero troncati in silenzio e la
cella few-shot diventerebbe indistinguibile da una zero-shot. È precisamente il
vincolo che il validatore verifica (si veda oltre). Le celle sono dunque
isolate sul fattore prompting, inteso come il suo effetto complessivo.

La lezione operativa che vale la pena estrarre riguarda il \emph{metodo} con cui
questo è stato accertato: confrontando meccanicamente le configurazioni risolte,
non leggendo la descrizione delle celle. Una differenza di configurazione può
entrare in una matrice sperimentale senza essere dichiarata, e in quel caso
nessun test la segnala.
...
Lo stato corrente è di $476$ test, tutti verdi (con una sola esclusione
dichiarata dal codice stesso). La loro distribuzione riflette la strategia
...
\subsection{Una diagnosi sbagliata, e la sua correzione}
\label{sec:test-ssh-ritrattazione}

Una precedente stesura di questa sezione dichiarava un test permanentemente
rosso, \texttt{test\_ssh\_alias\_no\_user\_no\_port}, attribuendone il
fallimento a una connessione SSH reale verso il cluster e quindi a una
dipendenza dalla rete.

Una stesura intermedia aveva ritrattato quella diagnosi, sostenendo che il test
fosse puramente locale perché ``cattura'' il comando costruito. \textbf{Anche la
ritrattazione era sbagliata}, e la ragione è la stessa che rende istruttivo
l'episodio. Il test sostituisce la classe \texttt{ClusterSSH} con una
sottoclasse che \emph{registra} il comando, ma che eredita il metodo di
esecuzione reale: la richiesta \texttt{POST /tick} arriva fino a
\texttt{subprocess.run(["ssh", \ldots, "gcluster", \ldots])}, e nel test non c'è
alcun mock di \texttt{subprocess}. Su una macchina in cui l'alias
\texttt{gcluster} è configurato, il test apre quindi una connessione vera e
tenta di eseguire sul cluster il comando \texttt{tick} dell'helper remoto;
altrove fallisce con un errore 502. In un'occasione documentata, eseguirlo dalla
workstation di sviluppo durante un addestramento ha coinciso con il blocco
delle connessioni per ``troppi login'' dell'utente sul cluster.

Entrambi gli errori sono nati dallo stesso schema: una conclusione tratta da
un'osservazione parziale (il nome del test la prima volta, l'oggetto che
cattura il comando la seconda), senza seguire la chiamata fino in fondo. È lo
schema che il Capitolo~\ref{cap:infrastruttura} documenta nei sette guasti
infrastrutturali. La correzione del test è semplice (un mock di
\texttt{subprocess.run}, già usato da un test vicino dello stesso file), ma
finché non è applicata il file \texttt{tests/test\_remote\_app.py} va escluso
dalle esecuzioni sulla workstation.

Il numero corrente è di $609$ test raccolti, dopo la rimozione dei test di
componenti eliminati e l'aggiunta di quelli introdotti dai lavori descritti in
questo capitolo e nel Capitolo~\ref{cap:ausiliari}: equivalenza del retrieval
ottimizzato, stimatori di pass@$k$, superficie di configurazione dell'eval,
cache e round-trip del servizio remoto, regressioni della pipeline degli
obiettivi ausiliari.

L'unico elemento non eseguito è un test dichiarato come tale dal codice
stesso, non un fallimento silenzioso.
