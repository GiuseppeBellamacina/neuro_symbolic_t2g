...
optimizes the model through reinforcement learning with 4 rule-based reward functions.

> **No neural reward model needed** — the reward is purely deterministic, computed from
> ROUGE-L similarity, bigram transition probabilities, format checks, and repetition penalties.
...
│   ├── rewards/
│   │   └── t2g_rewards.py             # 4 reward functions for GRPO
│   ├── training/
│   │   ├── grpo_t2g_train.py          # Main GRPO training loop (7-step pipeline)
│   │   ├── eval_t2g.py                # Checkpoint evaluation (ROUGE-L, exact match, bigram)
...
```bash
# Run all component tests (data → grammar → rewards → generation)
python main.py

# Test a single component
python main.py --task data       # Dataset + transition matrix
python main.py --task grammar    # Vocabulary mask + logits processor
python main.py --task rewards    # All 4 reward functions
```
...
- **Unsloth** *(optional GPU extra)*: [FastLanguageModel](https://docs.unsloth.ai/)
- **vLLM** *(optional GPU extra)*: [Inference engine](https://docs.vllm.ai/)
- **grammarllm**: Vendored constrained decoding library (PDA + LogitsProcessor)
- **Test Suite**: See [tests/REPORT.md](tests/REPORT.md) for full test inventory and results
